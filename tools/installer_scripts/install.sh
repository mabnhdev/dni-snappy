...
create_demo_gpt_partition()
{
    ...
    do
      demo_volume_label=$(echo $partition | cut -f3 -d: | sed -e 's/\\040/ /g')
    ...
    do
      ...
      if [ ! -z "$(echo $demo_volume_label | awk '{print $2}')" ] ; then
     ...
     if [ -e "$(echo $demo_volume_label | sed -e 's/ /_/g').tgz" ] ; then
...
# HACK to interoperate with "ubuntu-image" images.
if [ -f $system_boot_mnt/EFI/ubuntu/grub.cfg ] ; then
    mkdir -p $system_boot_mnt/EFI/ubuntu/grub
    cp $system_boot_mnt/EFI/ubuntu/grub.cfg $system_boot_mnt/EFI/ubuntu/grub
    cp $system_boot_mnt/EFI/ubuntu/grubenv $system_boot_mnt/EFI/ubuntu/grub
fi
